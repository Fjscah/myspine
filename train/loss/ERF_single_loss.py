...
class single_SpatialEmbLoss(nn.Module):

    def __init__(self, to_center=True, n_sigma=1, foreground_weight=1, width=256,height=256,resolution=256):
        super().__init__()
        ...
        # xm = torch.linspace(0, 1, width).view(
        #     1, 1, -1).expand(1, height, width)  # x-coordinates are within [0, 2] : (1, h, w)
        # ym = torch.linspace(0, 1, height).view(
        #     1, -1, 1).expand(1, height, width)  # y-coordinates are within [0, 1] : (1, h, w)
    ...
    def forward(self, prediction, labels,instances,  w_inst=1, w_var=10, w_seed=1, iou=False, iou_meter=None):
        ...
        labels : GT semantic-mask (N,1, h, w)'''
