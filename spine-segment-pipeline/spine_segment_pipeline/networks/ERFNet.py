...
class BranchedERFNet(BaseNet):
    ...
    def __init__(self, **kwargs):
        ...
        self.num_classes = kwargs["num_classes"]# dim for grad1,grad2,sigma, dim for bg,den,spine
        ...
        # shared encoder
        if (encoder is None):
            self.encoder = Encoder(sum(self.num_classes),inputchannel=self.inputchannel)  # Encoder(3+numclass)
        else:
            self.encoder = encoder
