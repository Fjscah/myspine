...
from .seg import segment,unetseg,trace,seg_base
...
def plugin_wrapper1():
   ...
   # update = Updater()
    ...
    def _peak_button():
        ...
        points=np.array(seg_base.get_point_from_mask(corners))
        if len(points)==0:
        ...
        if spotmethod=="hessian" or spotmethod=="structure":
            ...
            points=np.array(seg_base.get_point_from_mask(corners*adth2))
            points2=np.array(seg_base.get_point_from_mask(corners*~adth2))
        elif spotmethod=="CNN":
    ...
    def _grow_button():
        ...
        st=time.time()
        seedmask=seg_base.get_mask_from_point(points,img.shape) # cost too much ,to point cloud
        ...
        seedmask[ls<1]=0
        points=seg_base.get_point_from_mask(seedmask)
