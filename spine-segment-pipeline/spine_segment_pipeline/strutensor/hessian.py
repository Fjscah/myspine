...
def enhance_ridges(frames):
    ...
    if ndim==3:
        prominent_ridges=[]
        for frame in frames:
            prominent_ridge=enhance_ridges_2d(frame)
            prominent_ridges.append(prominent_ridge)
        prominent_ridges=np.array(prominent_ridges)
    else:
        prominent_ridges=enhance_ridges_2d(frames)
        
    
    
    return prominent_ridges
def enhance_ridges_2d(frame):
    """A ridge detection filter (larger hessian eigenvalue)"""
    shape=frame.shape
    ndim=frame.ndim
    if ndim==3:
